...
def get_schema_dataframe(
    ...
    for domain in schema.string_domain:
        domain_rows.append(
            [_add_quotes(domain.name), ", ".join(_add_quotes(v) for v in domain.value)]
        )
    ...
    for feature in schema.feature:
        ...
        if feature.HasField("domain"):
            ...
            domain_rows.append(
                [domain, ", ".join(_add_quotes(v) for v in feature.string_domain.value)]
            )

        feature_rows.append(
            [_add_quotes(feature.name), feature_type, feature_presence, valency, domain]
        )

    features = pd.DataFrame(
        feature_rows, columns=["Feature name", "Type", "Presence", "Valency", "Domain"]
